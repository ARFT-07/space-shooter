...
void iDraw() {
    ...
    if (strcmp(gameState, "menu") == 0) {
    ...
    else if (strcmp(gameState, "game") == 0 && score_count<=10) {
        // Background
        iSetColor(0, 0, 0);
        ...
        // Asteroids and Score
        for (int i = 0; i < NUM_ASTEROIDS; i++) {
            iShowImage(asteroidX[i], asteroidY[i], "assets/images/villain2.png");
            asteroidY[i] -= 8;
...
for (int i = 0; i < MAX_BULLETS; i++) {
        if (bulletVisible[i]==1) {
            iShowImage(bulletX[i], bulletY[i],"assets/images/bullets.png");
        }
    }
}
